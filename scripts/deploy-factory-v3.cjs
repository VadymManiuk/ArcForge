...
const outputPath = path.join(__dirname, "..", "deployment", "arcTestnet-v3.local.json");
...
async function main() {
  ...
  console.log(`V3 preflight passed for ${deployer.address}. Existing fees, profiles, and factories will be preserved.`);
  ...
  console.log(`V3 factory deployment submitted: ${deploymentTransaction.hash}`);
  ...
  const output = {
    ...
    status: "V3_CANDIDATE_DEPLOYED",
    curveModel: {
      version: 3,
      virtualUsdcReserve: 10_000,
      graduationThreshold: 40_000,
      curveInventorySoldPercent: 80,
      permanentLiquidityTvl: 80_000,
    },
    migration: {
      type: "FACTORY_ONLY",
  ...
  console.log(`V3 candidate manifest written to ${outputPath}`);
  console.log(`V3 Factory: ${factoryAddress}`);
  console.log("The registry was not changed. Run deploy:arc-testnet:v3:activate only after candidate verification passes.");
}
