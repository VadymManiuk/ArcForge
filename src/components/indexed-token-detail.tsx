...
export function IndexedTokenDetail({ address }: { address: string }) {
  ...
  return <div className="container-shell py-8">
    ...
    <div className="mt-6 grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="order-first grid h-fit gap-5 xl:order-none xl:sticky xl:top-24"><BuySellPanel token={token}/><Panel className="p-5"><p className="eyebrow">Indexed contracts</p><dl className="mt-4 grid gap-3 text-xs"><div className="flex justify-between"><dt className="text-slate-500">Token</dt><dd className="text-slate-300">{shortAddress(token.address)}</dd></div><div className="flex justify-between"><dt className="text-slate-500">Curve</dt><dd className="text-slate-300">{shortAddress(token.curveAddress ?? "")}</dd></div><div className="flex justify-between"><dt className="text-slate-500">Creator allocation</dt><dd className="text-slate-300">{token.creatorAllocationPercent?.toFixed(2)}%</dd></div><div className="flex justify-between"><dt className="text-slate-500">Launch block</dt><dd className="text-slate-300">{token.launchBlock}</dd></div></dl></Panel></aside>
    </div>
  </div>;
}
