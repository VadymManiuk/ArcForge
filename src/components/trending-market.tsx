...
export function TrendingMarket() {
  ...
  return <div className="container-shell pb-20">
    ...
    <div className="panel overflow-hidden">{ranked.map(({token,score},index) => {
      const onchainUnavailable = token.source === "onchain" && (isPartial || (loading && indexedTokens.length === 0));
      return <Link href={`/tokens/${token.address}`} key={token.address} className="grid items-center gap-4 border-b border-line/70 p-4 transition last:border-0 hover:bg-white/[.025] md:grid-cols-[56px_1.3fr_1fr_1fr_1fr_auto]">
        ...
        <div className="flex items-center gap-3"><TokenIcon label={token.icon}/><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-white">{token.name}</p><Badge tone={token.source === "onchain" ? isCached ? "neutral" : "good" : "neutral"}>{token.source === "onchain" ? isCached ? "Cached onchain" : "Onchain" : "Demo"}</Badge></div><p className="text-[10px] text-slate-500">{token.ticker}</p></div></div>
