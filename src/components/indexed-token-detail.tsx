...
export function IndexedTokenDetail({ address }: { address: string }) {
  const { tokens, loading, error, refresh } = useFactoryTokenIndex({ includeMarketData: false });
  const token = tokens.find((item) => item.address.toLowerCase() === address.toLowerCase());

  if (!token) return <div className="container-shell py-12"><Panel className="p-6"><p className="eyebrow">Factory index</p><h1 className="mt-3 text-2xl font-semibold text-white">{loading ? "Indexing Arc Testnet launch…" : "Launch not found"}</h1><p className="mt-3 text-sm text-slate-400">{error || (loading ? "Reading TokenLaunched and curve events without a simulated fallback." : "This address was not emitted by the configured ArcForge Factory.")}</p><div className="mt-5 flex gap-3">{!loading && <Button onClick={() => void refresh()}>Retry index</Button>}<Link href="/tokens" className="inline-flex h-10 items-center rounded-xl border border-line px-4 text-sm text-slate-300">Back to screener</Link></div>{error && <WarningBox>{error}</WarningBox>}</Panel></div>;
  ...
  return <div className="container-shell py-8">
    ...
    <div className="mt-6 grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="grid min-w-0 gap-5"><OnchainTokenDashboard token={token}/><Panel className="p-5"><p className="eyebrow">Risk profile</p><div className="mt-5 flex flex-wrap gap-2">{token.riskLabels.map((label) => <Badge key={label} tone={label.includes("high") || label.includes("missing") ? "bad" : "good"}><ShieldCheck className="mr-1 size-3"/>{label.replaceAll("_", " ")}</Badge>)}</div><WarningBox>Risk labels are informational and do not guarantee safety. Holder concentration remains unknown until Transfer indexing is enabled.</WarningBox></Panel></div>
