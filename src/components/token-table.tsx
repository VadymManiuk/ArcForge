...
export function TokenTable({ tokens, compact = false }: { tokens: TokenData[]; compact?: boolean }) {
  ...
  const shown = useMemo(() => tokens.filter((token) => {
    ...
    if (filter === "New" || filter === "Recently launched") return token.ageMinutes < 120;
